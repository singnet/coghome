...
from coghome.entity import Entity, entity_unavailable_state
...
class HomeState:
    def __init__(self, initial_state, queue_send, known_entity_ids):
        ...
        for entity_id, entity in self.entities.items():
            gon = GroundedObjectNode(entity_id, entity , unwrap_args = False)
            EvaluationLink(PredicateNode("has_id"), ListLink(gon, ConceptNode(entity_id)))
            InheritanceLink(ConceptNode(entity_id), ConceptNode("entity_id"))
            EvaluationLink(PredicateNode("has_domain"), ListLink(gon, ConceptNode(entity.get_domain())))
